...
| 2.3 | Giao diện người dùng (UI Components) | Cấu hình Preferences | Giao diện Cài đặt (Preferences Panel) | Quản lý cấu hình dịch thuật và phím tắt | Thiết lập cửa sổ preferences gồm tab: Chung (Cài đặt phím tắt), API OpenAI (Nhập Key, System Prompt, chọn model), Lịch sử (bật/tắt, xóa lịch sử).<br>Ước lượng: Dev: 3 ngày, QA: 1 ngày | Áp dụng cho: S-7, S-5, S-6. Lưu cấu hình an toàn trong NSUserDefaults hoặc SQLite. |
| 4.1 | Dịch thuật & Kết nối API | API Integration | Google Translate Client | Request dịch thuật qua API Google | Tích hợp thư viện hoặc wrapper gọi API Google Translate miễn phí để dịch nhanh văn bản.<br>Ước lượng: Dev: 1.5 ngày, QA: 0.5 ngày | Áp dụng cho: S-5. Không giới hạn ký tự và hoàn toàn miễn phí cho người dùng. |
| 4.2 | Dịch thuật & Kết nối API | API Integration | OpenAI GPT-4o Client | Request dịch thuật qua OpenAI API | Gọi API OpenAI GPT-4o sử dụng API Key cá nhân của người dùng, hỗ trợ tùy chỉnh System Prompt để dịch văn bản tự nhiên.<br>Ước lượng: Dev: 2 ngày, QA: 1 ngày | Áp dụng cho: S-5, S-7. Dịch chất lượng cao, lưu giữ thuật ngữ chuyên ngành. |
...
| M3: APIs Dịch thuật & Cài đặt | Tích hợp Google Translate API, OpenAI GPT-4o API và hoàn thiện Preferences Panel cùng SQLite lưu trữ lịch sử. | - Dịch thành công qua Google Translate and OpenAI GPT-4o.<br>- Lưu cấu hình OpenAI API Key cá nhân vào Preferences panel.<br>- Lịch sử dịch thuật được lưu vào SQLite cục bộ và hiển thị/xóa được từ giao diện cài đặt. |
...
| 2 | 2.1 (Popup dịch nhanh) | 4.1, 4.2 (API Integration) | FS | Cửa sổ HUD phải sẵn sàng để nhận dữ liệu dịch thuật từ Google/OpenAI và hiển thị lên cho người dùng. |
...
| --- | --- | --- | --- |
| DA-1 | Quyền truy cập API Google Translate miễn phí thông qua wrapper/web requests vẫn hoạt động ổn định và không bị chặn IP hàng loạt. | Hoạt động (Active) | Nếu bị chặn, người dùng bắt buộc phải dùng OpenAI API Key cá nhân để dịch thuật, làm giảm tính khả dụng miễn phí của app. |
...
| DA-3 | Khách hàng sẵn lòng tự cấu hình API Key OpenAI cá nhân của họ nếu muốn sử dụng dịch AI nâng cao. | Đã xác nhận (Confirmed) | Nếu không, chúng tôi sẽ phải thiết lập proxy server và hệ thống thanh toán để quản lý token, tăng chi phí và thay đổi hoàn toàn kiến trúc offline của app. |
