...
- Bản dịch sử dụng Gemini API (qua API key cá nhân của khách hàng) làm nguồn dịch chính trong MVP. Kiến trúc được thiết kế theo mô hình multi-provider mở, cho phép dễ dàng bổ sung thêm nguồn dịch (OpenAI, Google Translate, v.v.) ở các giai đoạn sau. Lịch sử dịch chỉ được lưu trữ cục bộ trong cơ sở dữ liệu SQLite dưới máy của người dùng.
...
| **Gemini API với kiến trúc multi-provider** | PP-1 (Dịch thuật chất lượng cao) | Dịch thuật AI chất lượng cao, hỗ trợ thuật ngữ chuyên ngành. Kiến trúc mở cho phép mở rộng thêm nguồn dịch trong tương lai. | Gemini API cung cấp khả năng dịch thuật ngữ cảnh tốt, chi phí hợp lý cho người dùng cá nhân. | Lập trình viên, Biên dịch viên |
...
| **S-5** | Client kết nối Gemini API (multi-provider architecture) | **Must-have** | CR4 | Thực hiện request dịch thuật qua Gemini API với key cá nhân của người dùng. Kiến trúc mở cho phép bổ sung provider sau. |
| **S-6** | Lưu trữ lịch sử cục bộ bằng SQLite | **Must-have** | CR5 | Lưu lịch sử dịch thuật dưới máy người dùng dưới dạng mã hóa cơ bản. |
| **S-7** | Giao diện cài đặt (Preferences View) | **Must-have** | CR4, CR5 | Cấu hình Gemini API Key cá nhân, chỉnh sửa phím tắt, bật tắt lưu lịch sử. |
...
| **O-2** | Tự cấp và chia sẻ API key miễn phí | Tránh phát sinh chi phí hóa đơn sử dụng AI khổng lồ cho nhà phát triển ứng dụng. |
...
| **FP-5** | Tích hợp Google Translate (miễn phí) | Khách hàng ưu tiên Gemini API trước, Google Translate bổ sung ở phase sau. |
| **FP-6** | Tích hợp OpenAI API | Bổ sung thêm provider dịch AI sau khi MVP ổn định với Gemini. |
...
```
+-------------------------------------------------------------+
| [ Chung ]     [ API Gemini ]      [ Phím Tắt ]    [ Lịch Sử ]|
+-------------------------------------------------------------+
| Cấu hình dịch thuật Gemini:                                  |
| API Key:   [ AIza...........................................]|
